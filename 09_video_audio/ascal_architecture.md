...
*   **Port Case Study**: For a detailed walkthrough of the VHDL-to-SystemVerilog migration process, see the [VHDL to SV Case Study](https://github.com/alfishe/vhdl-to-sv-casestudy), which documents the full ascal port with architectural decisions and syntax transformations. The case study includes a comprehensive verification methodology featuring **reference-based co-simulation** (VHDL as golden reference, 142 test cases, 1.8M+ pixel comparisons with 0 mismatches), **multi-category test coverage** (interpolation modes, pixel formats, scale factors, buffering modes, interlace handling, stress tests, fuzz testing, and clock domain crossing validation), and a **three-phase porting methodology** (structural translation → behavioral translation → verification with co-simulation and regression). Key lessons include the critical importance of pipeline register initialization matching and cycle-accurate comparison for catching subtle behavioral differences.
