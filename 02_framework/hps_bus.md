...
[`sys_top.v`](sys_top.md) hardware-abstraction layer to the [`hps_io.sv`](hps_io_module.md) command decoder
in every MiSTer core.  It is **not** an external board-level bus; it is
purely in-fabric wiring on the Cyclone V FPGA.

Source: [`sys_top.v`](https://github.com/MiSTer-devel/Template_MiSTer/blob/master/sys/sys_top.v), [`hps_io.sv`](https://github.com/MiSTer-devel/Template_MiSTer/blob/master/sys/hps_io.sv)

> [!NOTE]
> For the underlying GPO/GPI register mechanism that drives this bus from the HPS side,
> see [SPI-over-GPO Protocol](spi_protocol.md). For the full command opcode table,
> see [UIO Command Reference](../15_references/uio_command_reference.md).
...
In [`sys_top.v`](https://github.com/MiSTer-devel/Template_MiSTer/blob/master/sys/sys_top.v):
```verilog
// sys_top.v — GPO/GPI cross-point feeding HPS_BUS[35:0]
...
> [!WARNING]
> The two-register synchroniser means there is a **2× `clk_sys` latency** between
> the HPS writing a GPO word and the FPGA seeing the strobe edge. Cores running at
> very low `clk_sys` frequencies may experience visible stalls during ROM loading.
> The `ioctl_wait` back-pressure mechanism exists specifically for this case.
...
[`hps_io.sv`](https://github.com/MiSTer-devel/Template_MiSTer/blob/master/sys/hps_io.sv) exposes an `EXT_BUS[35:0]` port for core-specific extensions.
...
```verilog
// hps_io.sv — EXT_BUS pass-through and response mux
...
> [!NOTE]
> For Minimig-specific EXT_BUS usage (IDE registers, CDDA streaming), see
> [Core Catalog — Minimig](../05_core_catalog/minimig.md). For the original Amiga
> hardware architecture, see [Amiga Bootcamp](https://github.com/alfishe/amiga-bootcamp).

---

## Platform Context

| Platform | HPS↔FPGA Communication |
|---|---|
| **MiSTer** | Software SPI over GPO/GPI registers → HPS_BUS 49-bit vector |
| **Analogue Pocket** | APF bus — proprietary bridge, undocumented internals |
| **MiST** | SPI peripheral (dedicated HW SPI, not software-emulated) |
| **MiSTeX** | AXI bridge (Zynq PS↔PL) — higher bandwidth, different protocol |
| **Software emulation** | N/A — all in-process memory access |

MiSTer's software SPI approach sacrifices raw bandwidth for simplicity — any core developer
can understand the protocol by reading `hps_io.sv`, and no platform-specific SPI IP is needed.
The manual 16× unrolled block transfers in [`fpga_io.cpp`](https://github.com/MiSTer-devel/Main_MiSTer/blob/master/fpga_io.cpp)
achieve ~100–150 MB/s, which is sufficient for ROM loading but tight for real-time CD audio streaming.
